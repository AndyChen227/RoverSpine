...
- `[ ]` **Rev A — schematic complete, footprints not assigned.** 14 nets, netlist check `ALL PASS` against [the net structure](../../docs/roadmap.md#nets). **ERC 44 / 0** — the 30 unused Pi-header pins by design, plus 14 symbols with no footprint. Layout not started, not fabricated. **Parts bought 2026-09-13**, not yet delivered, so D50A pin 1 is still an assumption.
- **Updated 2026-09-14.** Six of those fourteen symbols — `R7`–`R12` — had carried a **160-pin QFP footprint** since 09-13, committed and pushed, while every document here said "footprints not assigned". Cleared, and a **footprint check added** to the checker, because ERC's filter cannot tell an 0805 from a through-hole resistor. See [the devlog](../../docs/devlog/2026-09-14-a-160-pin-qfp-on-a-resistor.md).

`[ ]` **Rev A — 原理图完工，封装未分配。** 14 个网络，网表核对 `ALL PASS`。**ERC 44 / 0**——30 条是排针上没用到的引脚（设计如此），另外 14 条是没有封装的符号。布局未开始，未打样。**物料已于 2026-09-13 买齐**，尚未到货，所以 D50A 的 1 号脚仍是假设。

**2026-09-14 更新。** 那 14 个符号里有 6 个（`R7`–`R12`）从 09-13 起就带着一个 **160 脚 QFP 封装**，已提交并推送，而这里每一份文档当时都写着"封装未分配"。现已清除，并给核对脚本**加上了封装检查**——因为 ERC 的筛选规则分不出 0805 和通孔电阻。见[开发日志](../../docs/devlog/2026-09-14-a-160-pin-qfp-on-a-resistor.md)。
...
| A | 2026-09-14 | `[ ]` schematic done | **No change to the circuit.** Six of those 14 were not "empty" — `R7`–`R12` had carried `Package_QFP:PQFP-160_28x28mm_P0.65mm` since `949f91a`, propagated by the same copy-paste that made the six dangling wire ends. **Clearing them did not change the ERC count: 44 before, 44 after.** Footprint comparison added to [the checker](tools/README.md), because `R_*` matches the 0805 and the axial footprint equally — **swapping the series resistors with the pull-downs passes ERC at a clean 30 / 0.** The 09-13 design rules are committed and `dimension_units` reverted to KiCad's default. See [the devlog](../../docs/devlog/2026-09-14-a-160-pin-qfp-on-a-resistor.md) |
...
6. **Assign the fourteen footprints.** **Two completion tests, not one:** ERC at exactly **30 / 0**, *and* [the checker](tools/README.md) at **`ALL PASS`** including its footprint block. ERC alone is not enough — `R1`–`R12` all declare the filter `R_*`, so an 0805 on a series resistor passes it silently.
